...
def check_valid_files(input_dir):
    ...
    required_xpts = ["mi.xpt"]
    optional_xpts = ["dm.xpt", "ds.xpt", "ex.xpt", "ts.xpt"]
    ...
    # Check if MI.xpt exists
    mi_found = len(set(required_xpts) & set(input_xpt_cleaned))
...
def MI_dataframe(xpt_dir, json_file):
    ...
    mi_xpt = utils.read_XPT(xpt_dir, "/mi.xpt")
    ...
    if len(dfMI) != len(mi_xpt):
        ...
        LOG.warning(
            str(MIdiff)
            + " rows are not General Histopathologic Exam, Qual and will be excluded"
        )
...
def TS_dataframe(xpt_dir, json_file): 
    ...
    TS_xpt = utils.read_XPT(xpt_dir, "/ts.xpt")
...
def DS_dataframe(xpt_dir, json_file):
    ...
    ds_xpt = utils.read_XPT(xpt_dir, "/ds.xpt")
...
def DM_dataframe(xpt_dir, json_file):
    ...
    dm_xpt = utils.read_XPT(xpt_dir, "/dm.xpt")

...
getAnimalsSex<-function(dbToken,
  ...
  animalListSEX <-
    genericQuery(dbToken,
                 "select studyid
                        ...
                        ,sex
                  from dm
                  where dm.usubjid in (:1)",
                 animalList[,c('USUBJID')])
  ...
  if (inclUncertain) {
    ...
    foundAnimals <-
      animalListSEX[toupper(trimws(SEX)) %in% toupper(trimws(sexFilter)) | ! toupper(trimws(SEX)) %in% ctSEX,
                    list(STUDYID, USUBJID, SEX,
                         UNCERTAIN_MSG=ifelse(! toupper(trimws(SEX)) %in% ctSEX ,
  ...
  if ("UNCERTAIN_MSG.x" %in% names(foundAnimals)) {
    # An UNCERTAIN_MSG column is included in both input and extratced list of animals
